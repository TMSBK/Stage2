    ...
    **/
    ...
    static createRestaurantsStore(restaurants) {
        ...
        open.onerror = function(err) {
            console.error('IndexeDB error: ' + err.target.errorCode); // eslint-disable-line
        };
    ...
    static getCachedData(callback) {
        var restaurants = []; // eslint-disable-line
        // Get the compatible IndexedDB version
        var indexedDB = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB || window.shimIndexedDB;
        var open = indexedDB.open('RestaurantDataBase', 1);
        open.onsuccess = function() {
    ...
    static getCachedReviews(restaurantId, callback) {
        var reviews = []; // eslint-disable-line
        // Get the compatible IndexedDB version
        var indexedDB = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB || window.shimIndexedDB;
        var open = indexedDB.open('RestaurantDataBase', 1);
        open.onsuccess = function() {
    ...
    */
    ...
    static fetchRestaurants(callback) {
        if (navigator.onLine) {
            ...
            console.log('Browser Offline - Using cached data!'); // eslint-disable-line
    ...
    */
    ...
    static addRestaurantToFavorites(restaurantId, isFav, callback) {
        ...
        fetch(url, { method: 'put' })
            .then(response => callback(null, 1)) // eslint-disable-line
